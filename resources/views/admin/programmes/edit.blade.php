    ...
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Update program details, stories, and focus areas.</p>
            </div>
        ...
        <form id="program-form" method="POST" action="{{ route('admin.programmes.update', $programme) }}" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            @csrf
            @method('PUT')
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6 shadow-sm space-y-4">
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $programme->title) }}" required class="w-full text-xl px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-acef-green">
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Description</label>
                        <x-rich-text-editor name="content" :value="old('content', $programme->content)" />
                    </div>

                    <div>
                        <label for="excerpt" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Excerpt</label>
                        <textarea name="excerpt" id="excerpt" rows="3" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white">{{ old('excerpt', $programme->excerpt) }}</textarea>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6 shadow-sm" x-data="{ 
                    stories: {{ json_encode($programme->impact_stories ?? []) }},
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6 shadow-sm" x-data="{ 
                    areas: {{ json_encode($programme->focus_areas ?? []) }},
                    newArea: '',
            ...
            <div class="space-y-6">
                 ...
                 <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-5 shadow-sm">
                    ...
                    <div>
                        ...
                        <select name="status" id="status" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-acef-green text-sm">
                            <option value="draft" {{ $programme->status === 'draft' ? 'selected' : '' }}>Draft</option>
                            <option value="published" {{ $programme->status === 'published' ? 'selected' : '' }}>Published</option>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-5 shadow-sm">
                    ...
                    <div x-data="{ 
                        ...
                        <input type="hidden" name="image" id="image_path" value="{{ $programme->image }}">
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-5 shadow-sm space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="category" value="{{ old('category', $programme->category) }}" placeholder="e.g. Agriculture" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 text-sm">
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-gray-500 mb-1">Icon (Emoji or SVG)</label>
                        <input type="text" name="icon" value="{{ old('icon', $programme->icon) }}" placeholder="e.g. 🌱" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 text-sm">
                    </div>
                    ...
                    <div>
                        ...
                        <div x-data="{
                            ...
                            selectedCountries: {{ json_encode($programme->country ?? []) }},
                            search: '',
                            filteredCountries() {
                    ...
                    <div>
                        ...
                        <input type="text" name="duration" value="{{ old('duration', $programme->duration) }}" placeholder="e.g. 5 Years" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 text-sm">
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-gray-500 mb-1">Location (Text Detail)</label>
                        <input type="text" name="location" value="{{ old('location', $programme->location) }}" placeholder="e.g. Rift Valley, Kenya" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 text-sm">
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-1">Funding Goal ($)</label>
                            <input type="number" name="funding_goal" value="{{ old('funding_goal', $programme->funding_goal) }}" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-1">Raised So Far ($)</label>
                            <input type="number" name="funding_raised" value="{{ old('funding_raised', $programme->funding_raised) }}" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 text-sm">
                        </div>
                    </div>
                    ...
                    <div class="space-y-3 pt-2 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div class="col-span-2">
                                ...
                                <input type="text" name="stats[beneficiaries]" value="{{ old('stats.beneficiaries', $programme->stats['beneficiaries'] ?? '') }}" class="w-full px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-xs">
                            </div>
                            <div>
                                <label class="block text-[10px] font-medium text-gray-500 mb-1">Trees (e.g. 50,000)</label>
                                <input type="text" name="stats[trees]" value="{{ old('stats.trees', $programme->stats['trees'] ?? '') }}" class="w-full px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-xs">
                            </div>
                            <div>
                                <label class="block text-[10px] font-medium text-gray-500 mb-1">Communities (e.g. 12)</label>
                                <input type="text" name="stats[communities]" value="{{ old('stats.communities', $programme->stats['communities'] ?? '') }}" class="w-full px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-xs">
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-500 mb-1">Factsheet (PDF)</label>
                         <div x-data="{ 
                            fileName: '{{ $programme->factsheet ? basename($programme->factsheet) : '' }}',
                            selectFile() {
                                window.openMediaPicker((item) => {
                            ...
                            <div class="flex gap-2">
                                <input type="hidden" name="factsheet" id="factsheet_path" value="{{ $programme->factsheet }}">
