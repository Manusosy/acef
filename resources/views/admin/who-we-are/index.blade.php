<x-app-dashboard-layout>
    <x-slot name="header">Who We Are Images</x-slot>
    <x-slot name="title">Manage Carousel</x-slot>
    ...
    <div class="max-w-5xl">
        ...
        <div id="imagesContainer" class="space-y-4" x-data="{ 
            ...
            @forelse($images as $image)
                <div class="bg-white dark:bg-gray-800 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm transition-all group" :class="editingId === {{ $image->id }} ? 'ring-2 ring-emerald-500' : ''">
                    <div class="flex items-start gap-6 p-4">
                        ...
                        <div class="w-40 h-28 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-900 flex-shrink-0 border border-gray-200 dark:border-gray-700">
                            @if($image->media)
                                <img src="{{ $image->media->url }}" alt="" class="w-full h-full object-cover">
                            @else
                        ...
                        <!-- Content -->
                        <div class="flex-1 min-w-0" x-show="editingId !== {{ $image->id }}">
                            <div class="flex items-center gap-3 mb-2">
                                ...
                                <span class="px-2 py-0.5 text-[10px] font-bold uppercase rounded-full {{ $image->is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30' : 'bg-gray-200 text-gray-600 dark:bg-gray-600' }}">
                                    {{ $image->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </div>
                            <p class="text-sm text-gray-600 dark:text-gray-300 line-clamp-2">{{ $image->caption ?: 'No caption provided' }}</p>
                        </div>

                        <!-- Edit Form -->
                        <div class="flex-1 min-w-0" x-show="editingId === {{ $image->id }}" x-cloak>
                            <form id="image-form-{{ $image->id }}" class="space-y-4">
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Country</label>
                                        <input type="text" name="country" value="{{ $image->country }}" class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-emerald-500 focus:border-emerald-500" placeholder="e.g. Kenya">
                                    </div>
                                    <div>
                                ...
                                <div>
                                    ...
                                    <textarea name="caption" rows="2" class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-emerald-500 focus:border-emerald-500" placeholder="Image description...">{{ $image->caption }}</textarea>
                                </div>
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <button type="button" @click="updateImage({{ $image->id }})" class="px-3 py-1.5 text-xs font-medium bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg">Save Changes</button>
                                </div>
                            </form>
                        </div>

                        <!-- Actions -->
                        <div class="flex flex-col items-center gap-2" x-show="editingId !== {{ $image->id }}">
                            <button @click="editingId = {{ $image->id }}" class="p-2 text-gray-400 hover:text-emerald-600 transition-colors rounded-lg hover:bg-emerald-50 dark:hover:bg-emerald-900/20" title="Edit">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </button>
                            <button onclick="deleteImage({{ $image->id }})" class="p-2 text-gray-400 hover:text-red-600 transition-colors rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20" title="Delete">
    ...
    <script>
        ...
        window.addImage = function(media) {
            ...
            fetch('{{ route("admin.who-we-are.store") }}', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' },
        ...
        new Sortable(document.getElementById('imagesContainer'), {
            ...
            onEnd: function() {
                ...
                fetch('{{ route("admin.who-we-are.reorder") }}', {
                    method: 'POST',
