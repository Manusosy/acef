<x-app-dashboard-layout>
    <x-slot name="header">Edit Resource</x-slot>
    ...
    <div class="max-w-4xl">
        ...
        <form action="{{ route('admin.resources.update', $resource) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" value="{{ old('title', $resource->title) }}" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Description</label>
                    <textarea name="description" rows="3" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white">{{ old('description', $resource->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="type" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                        @foreach(['Report', 'Policy', 'Publication', 'Guide'] as $type)
                ...
                <div>
                    ...
                    <input type="text" name="category" value="{{ old('category', $resource->category) }}" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Year *</label>
                    <input type="text" name="year" value="{{ old('year', $resource->year) }}" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                </div>
                ...
                <div>
                    ...
                    <div x-data class="flex-1">
                         ...
                         <div class="relative">
                             <input type="text" name="file_path" id="file_path" value="{{ $resource->file_path }}" placeholder="Click to select file..." 
                    ...
                    @if($resource->file_path)
                        <p class="text-xs text-gray-500 mt-1">Current file: {{ basename($resource->file_path) }} ({{ $resource->size }})</p>
                    @endif
                </div>

                <div class="md:col-span-2 pt-4">
                    <label class="flex items-center gap-3">
                        <input type="checkbox" name="is_locked" value="1" {{ $resource->is_locked ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 text-acef-green focus:ring-acef-green">
