    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <h1 class="text-3xl font-black text-gray-900 tracking-tight">Edit Project</h1>
                <p class="text-gray-500">Update project details, manage media, and track progress.</p>
            </div>
        ...
        <form id="editProjectForm" action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                <div>
                    <h2 class="text-2xl font-extrabold text-gray-900">Project Details</h2>
                    <p class="text-gray-500 text-sm">Provide the core information about the project, including its title, category, and associated program.</p>
                </div>
                
                <div class="space-y-6">
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Project Title *</label>
                        <input type="text" name="title" value="{{ old('title', $project->title) }}" placeholder="e.g. Community Reforestation Initiative 2024" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            ...
                            <input type="number" name="start_date" placeholder="2024"  value="{{ old('start_date', $project->start_date?->format('Y')) }}"
                    ...
                    <div>
                        ...
                        <div x-data="{
                            ...
                            selectedCountries: {{ json_encode(old('country', $project->country ?? [])) }},
                            search: '',
                            filteredCountries() {
            ...
            <!-- Objectives (Repeater) -->
            <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6" x-data="{ objectives: {{ json_encode(old('objectives', $project->objectives ?? [''])) }} }">
            ...
            <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6" x-data="{ 
                voices: {{ json_encode(old('voices', $project->voices ?? [])) }},
                ...
                <div class="space-y-4">
                    <template x-for="(voice, index) in voices" :key="index">
                     ...
                     <div x-show="voices.length === 0" class="text-center py-4 text-gray-400 text-sm italic">No testimonials added yet.</div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                    ...
                    <div class="space-y-8">
                        <!-- Featured Image -->
                        <div x-data="{ preview: '{{ $project->image ? Storage::url($project->image) : '' }}', mediaId: null, 
                            selectFromLibrary() {
                        ...
                        <!-- Supporting Partners (Dropdown) -->
                         <div x-data="{ 
                            open: false,
                            selectedCount: {{ $project->partners->count() }},
                            updateCount() {
                ...
                <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                    ...
                    <div x-data="{ 
                        gallery: {{ json_encode(old('gallery', $project->gallery ?? [])) }},
                        selectGalleryImage() {
                            if (window.openMediaPicker) {
                                window.openMediaPicker((media) => {
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div class="space-y-6">
                        <div>
                            ...
                            <input type="number" name="goal_amount" value="{{ old('goal_amount', $project->goal_amount) }}" 
                        ...
                        <div>
                            ...
                            <input type="number" name="raised_amount" value="{{ old('raised_amount', $project->raised_amount) }}" 
                                   class="w-full px-5 py-3.5 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 text-gray-900 font-bold">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Video URL</label>
                            <input type="url" name="video_url" value="{{ old('video_url', $project->video_url) }}" placeholder="https://youtube.com/..." 
                     ...
                     <div class="space-y-6">
                        @if(auth()->user()->isAdmin())
                         <div>
                            ...
                            <select name="status" class="w-full px-5 py-3.5 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 text-gray-900 cursor-pointer">
                                <option value="ongoing" {{ old('status', $project->status) == 'ongoing' ? 'selected' : '' }}>Active / Ongoing</option>
                                <option value="starting" {{ old('status', $project->status) == 'starting' ? 'selected' : '' }}>Starting / Planned</option>
                        ...
                        <div class="pt-4 border-t border-gray-100 text-left">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $project->is_featured) ? 'checked' : '' }} class="rounded border-gray-300 text-emerald-500 focus:ring-emerald-500">
