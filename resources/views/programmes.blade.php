...
<head>
    ...
    <title>{{ __('pages.programmes.hero_title') }} - ACEF</title>
    ...
    <main class="relative z-20 -mt-10">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-6">
                @foreach(__('pages.programmes.stats') as $stat)
                    <div class="bg-white p-8 rounded-3xl shadow-xl border border-gray-100 flex flex-col space-y-2">
                        @if($stat['icon'] == 'blue')
                            <span class="text-acef-green"><svg class="w-6 h-6" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                        ...
                        @elseif($stat['icon'] == 'orange')
                            <span class="text-acef-green text-xl">🤝</span>
                        @endif
                        <h4 class="text-gray-400 text-sm font-bold uppercase tracking-wider">{{ $stat['label'] }}</h4>
                        <span class="text-4xl font-black text-acef-dark">{{ $stat['value'] }}</span>
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
                <div class="flex flex-col md:flex-row justify-between items-center gap-8">
                    <h2 class="text-4xl font-black text-acef-dark tracking-tighter">
                        {{ __('pages.programmes.initiatives_title') }}</h2>

                    <div class="flex flex-wrap items-center gap-4">
                        <!-- Basic Controls -->
                        <button
                            class="flex items-center space-x-2 px-6 py-3 bg-gray-50 rounded-xl font-bold text-gray-500 hover:bg-gray-100 transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z">
                                </path>
                            </svg>
                            <span>{{ __('pages.programmes.filter_btn') }}</span>
                        </button>
                        <button
                            class="flex items-center space-x-2 px-6 py-3 bg-gray-50 rounded-xl font-bold text-gray-500 hover:bg-gray-100 transition-colors">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <span>{{ __('pages.programmes.sort_btn') }}</span>
                        </button>
                        <div class="relative">
                            <input type="text" placeholder="{{ __('pages.programmes.search_placeholder') }}"
                                class="pl-12 pr-6 py-3 bg-gray-50 border-none rounded-xl w-64 focus:ring-2 focus:ring-acef-green transition-all">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach(__('pages.programmes.list') as $item)
                        <div
                            class="bg-white p-10 rounded-3xl border border-gray-100 shadow-sm hover:shadow-2xl transition-all group flex flex-col justify-between min-h-[400px]">
                            <div class="space-y-6">
                                <div class="flex justify-between items-start">
                                    <div
                                        class="w-16 h-16 bg-acef-green/10 rounded-2xl flex items-center justify-center text-3xl">
                                        {{ $item['icon'] }}
                                    </div>
                                    <span
                                        class="text-acef-green text-[10px] font-bold uppercase tracking-widest">{{ $item['category'] }}</span>
                                </div>
                                <div class="space-y-3">
                                    <h3
                                        class="text-2xl font-black text-acef-dark group-hover:text-acef-green transition-colors">
                                        {{ $item['title'] }}</h3>
                            ...
                            <div class="flex justify-between items-center pt-6 border-t border-gray-50">
                                ...
                                <a href="{{ route('programmes') }}"
                                    class="text-acef-green font-bold text-sm flex items-center group-hover:translate-x-1 transition-transform">
